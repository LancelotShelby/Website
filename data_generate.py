...
try:
    ...
    print(f"Error: Missing file. Make sure '{e.filename}' is in the same directory as the script.")
    exit()
...
for uni in universities_data:
    ...
    uni['country_slug'] = slugify(uni['country'])
    ...
    uni['internal_url'] = os.path.join(OUTPUT_DIR, uni['country_slug'], uni['slug'], f"{uni['slug']}_university.html").replace('\\', '/')
    
    uni['explore_url'] = f"unis/{uni['country_slug']}/{uni['slug']}/{uni['slug']}_university.html"
...
contact_output_path = 'contact.html'
with open(contact_output_path, 'w', encoding='utf-8') as f:
    f.write(contact_template.render(
        relative_path_prefix=ROOT_PREFIX,
        country_universities_by_type=country_universities_by_type
    ))
print(f"Generated {contact_output_path}")

# --- NEW: Generate Country Pages ---
...
for country in countries_data:
    ...
    # Render the main country page
    country_page_filename = f"{country_slug}.html"
    country_page_path = os.path.join(country_dir, country_page_filename)

    with open(country_page_path, 'w', encoding='utf-8') as f:
        f.write(country_detail_template.render(
            country=country,
            country_name=country_name,
            country_universities_by_type=country_universities_by_type, # <-- Added this line
            relative_path_prefix=TWO_LEVEL_PREFIX
        ))
    print(f" Generated {country_page_path}")
# --- Generate private and public university pages per country ---
for country in countries_data:
    country_slug = country['slug']
    country_dir = os.path.join(OUTPUT_DIR, country_slug)
    
    public_unis = [uni for uni in universities_data if uni['country_slug'] == country_slug and uni['type'].lower() == 'public']
    private_unis = [uni for uni in universities_data if uni['country_slug'] == country_slug and uni['type'].lower() == 'private']
    ...
    if public_unis:
        ...
        with open(public_path, 'w', encoding='utf-8') as f:
            f.write(country_public_template.render(
                country=country,
                unis=public_unis,
                country_universities_by_type=country_universities_by_type, # <-- Corrected line
                relative_path_prefix=TWO_LEVEL_PREFIX
            ))
